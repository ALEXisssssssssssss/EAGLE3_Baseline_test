...
class Model(nn.Module):
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        ...
        for i in range(depth):
            ...
            # MODIFIED for [SVIP]
            entropy = self.calc_entropy(last_headout)  # shape: (top_k,)
            # calc_entropy = entropy.mean()
            # calc_entropy = entropy.max()
            calc_entropy = entropy.min()
            if torch.sqrt(calc_entropy * 0.15) > self.entropy_thershold:
                break

...
class Model(nn.Module):
    ...
    @torch.no_grad()
    def topK_genrate_spec_plus(self, hidden_states, input_ids, head, logits_processor, spec_plus_ffn):
        ...
        for i in range(depth):
            ...
            if i == 5 or i == 8 or i == 11:
                hidden_states_to_classify = out_hidden[0]
                device = next(spec_plus_ffn.parameters()).device
                hidden_states_to_classify = hidden_states_to_classify.to(device)
                hidden_states_to_classify = hidden_states_to_classify.to(torch.float32)
                
                keep_scores = torch.sigmoid(spec_plus_ffn(hidden_states_to_classify))

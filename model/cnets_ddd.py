...
class Model(nn.Module):
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        ...
        # [MODIFIED FOR DDD] 初始化 logprobsum = top-k第一层概率
        logprobsum = scores.clone()
        ...
        for i in range(depth):
            ...
            # [MODIFIED FOR DDD] --- 动态熵判断
            if S is not None and x is not None and (i + 1) in S:
                # H = log(sum_i exp(logprobsum[i])) - max
                max_logprob = torch.max(logprobsum)
                sum_exp = torch.sum(torch.exp(logprobsum - max_logprob))
                H = torch.log(sum_exp) + max_logprob  # logsumexp
                if H.item() < x:
                    break
            ...
            # [MODIFIED FOR DDD] --- 更新 logprobsum
            new_logprobs = topk_p.view(-1)[topk_cs_index] 
            logprobsum = logprobsum[out_ids] + new_logprobs

...
try:
    ...
    from utils_c import *
    from choices import *
    from EAGLE.eagle.model.utils0 import prepare_logits_processor
...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True, total_tokens=60, depth=8, top_k=10, threshold=1.0):
        super().__init__()
        self.config=config
        ...
        self.gamma = depth
        self.gammamin = depth/2
        self.gammamax = depth*1.5
        self.gammadelta = 1
        self.gammabar = depth
        self.gammaeta = 0.2
    ...
    @torch.no_grad()
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor,accept_length):
        ...
        # 4
        if accept_length != None:
            if self.gamma == accept_length:
                self.gamma = accept_length + self.gammadelta
            self.gammamax = max(self.gamma,self.gammamax)
            self.gammabar = min(self.gammamax,max(self.gammamin,(1-self.gammaeta)*self.gammabar+self.gammaeta*accept_length))
            self.gamma = math.ceil(self.gammabar)
            self.gammamin = min(self.gammamin,self.gamma)
            depth = self.gamma

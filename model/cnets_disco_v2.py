...
class Model(nn.Module):
    ...
    @torch.no_grad()
    def topK_genrate_disco(self, hidden_states, input_ids, head, logits_processor, disco_ffn):
        ...
        #scores_list.append(scores.unsqueeze(0))
        ...
        for i in range(depth):
            ...
            cu_scores = topk_p + scores[:, None] # shape: 10*10
            
            
            if i == 2 or i == 5 or i == 8 or i == 11:
                features_list = []
                depth_feature = torch.tensor([(i + 1.0) / 10], device=topk_p.device)
                for parent_idx in range(top_k):
                    probs_log = topk_p[parent_idx][:top_k]  # 已是logsoftmax，无需还原
                    entropy = -torch.sum(torch.exp(probs_log) * probs_log, dim=-1, keepdim=True)  # scalar 熵，使用 log-softmax 计算
                    feature = torch.cat([
                        probs_log,       # log-softmax vector [top_k]
                        entropy,         # shape: [1]
                        depth_feature    # shape: [1]
                    ], dim=0)  # shape: [top_k + 2]，通常是 [10 + 2]
                    features_list.append(feature)

                features = torch.stack(features_list, dim=0)  # shape: [top_k * 1, feature_dim]
                features = features.to(next(disco_ffn.parameters()).device)

                keep_scores = torch.sigmoid(disco_ffn(features))  # shape: [top_k]
                keep_mask = keep_scores > 0.5    

                keep_mask = keep_mask.unsqueeze(1).expand(-1, top_k)     # shape: [top_k, top_k]
                keep_mask = keep_mask.reshape(-1)  

                cu_scores = cu_scores.view(-1)
                num_true = keep_mask.sum().item()
                if num_true >= 10:
                    cu_scores[~keep_mask] = -float("inf")

                topk_cs = torch.topk(cu_scores, top_k, dim=-1)
            ...
            #scores_list.append(cu_scores.view(1, -1))
            scores_list.append(cu_scores.view(-1))
